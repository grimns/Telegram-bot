...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    m = re.match(r"^(private|vip)_(month|year|forever)_country_kaz$", data) or re.match(r"^(private|vip)_(month|year|forever)_country_kaz$", data)
    if m:
        prefix_pack = f"{m.group(1)}_{m.group(2)}"
    ...
    # Russia -> FK Wallet (with instructions)
    ...
    if m:
        ...
        instr = (f"💰 FK Wallet\nСумма: {price_display(base,dur)}\nСсылка: {FKWALLET_LINK}\n\n"
                 f"Инструкция:\n1) Зайдите в кабинет → Кошелёк → Рубли → Вывод\n"
                 f"2) Вставьте этот номер: `{FKWALLET_NUMBER}`\n3) Укажите сумму и подтвердите перевод\n\n"
                 "Скиньте скрин игры после оплаты.")
