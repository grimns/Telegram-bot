...
# --------------------- ТВОИ ДАННЫЕ (оставил те что ты прислал) ---------------------
...
# pending_users:
# { user_id: { 'state': 'awaiting_screenshot'|'support', 'pack': 'private_month', 'method': 'usdt_trc' } }
pending_users = {}
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if m:
        ...
        # price summary
        usd = USD_PRICES[base][dur]              # <- исправлено
    ...
    # ====== Stars invoice (автоматическая выдача при оплате через invoice) ======
    ...
    if m:
        ...
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_ton")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_usdt_trc")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
            f"💵 Оплата USDT TRC20\nСумма: {USD_PRICES[base][dur]}$\nАдрес: `{USDT_TRC20}`\n\nСкиньте скрин игры после оплаты.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(kb))
        return
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_usdt_erc")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
            f"💵 Оплата USDT ERC20\nСумма: {USD_PRICES[base][dur]}$\nАдрес: `{USDT_ERC20}`\n\nСкиньте скрин игры после оплаты.",
            parse_mode="Markdown",
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_cryptobot")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
    ...
    # country methods -> Kazakhstan and others
    m = re.match(r"^(private|vip)_(month|year|forever)_country_kaz_others$", data)
    if m:
        prefix_pack = f"{m.group(1)}_{m.group(2)}"
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_uk_donatello")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_uk_donatalerts")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_ru_donatalerts")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
            f"💰 Donat Alerts\nСумма: {USD_PRICES[base][dur]}$\nСсылка: {DONATALERTS_LINK}\n\nСкиньте скрин игры после оплаты.",
            reply_markup=InlineKeyboardMarkup(kb))
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_ru_fkwallet")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        instr = (f"💰 FK Wallet\nСумма: {USD_PRICES[base][dur]}$\nСсылка: {FKWALLET_LINK}\n\n"
                 f"📞 Номер для перевода: `{FKWALLET_NUMBER}`\n\n"
                 "Инструкция:\n1) Зайдите в кабинет → Кошелёк → Рубли → Вывод\n"
                 "2) Вставьте этот номер\n3) Укажите сумму и подтвердите перевод\n\n"
                 "Скиньте скрин игры после оплаты.")
        await query.message.reply_text(instr, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
        return
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = [[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_kz_donatalerts")],
              [InlineKeyboardButton("⬅️ Назад", callback_data="back")]]
        await query.message.reply_text(
            f"💰 Donat Alerts (Казахстан/Другие)\nСумма: {USD_PRICES[base][dur]}$\nСсылка: {DONATALERTS_LINK}\n\nСкиньте скрин игры после оплаты.",
            reply_markup=InlineKeyboardMarkup(kb))
        return

    # ====== Пользователь нажал "Я оплатил" (в любом варианте) ======
    if data.startswith("paid_"):
        # structure: paid_{pack}_{method...}
        payload = data.replace("paid_", "")
        # pack is first two parts e.g. private_month
        parts = payload.split("_")
        if len(parts) < 3:
            await query.message.reply_text("Ошибка формата оплаты, попробуйте ещё раз или выберите другой способ.")
            return
        pack = f"{parts[0]}_{parts[1]}"
        method = "_".join(parts[2:])  # e.g. usdt_trc or uk_donatello etc.
    ...
    if data.startswith("give_"):
        ...
        if target_id in pending_users:
            ...
            try:
                await context.bot.send_message(target_id, f"✅ Оплата подтверждена!\nВот ссылка на канал:\n{link}")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_users:
        ...
        if state == "awaiting_screenshot":
            ...
            try:
                # forward photo to admin (send as photo so admin sees it)
