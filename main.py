# main.py — полнофункциональный бот с оплатами, регионами и поддержкой
# Примечание: вставь свой TOKEN и ADMIN_ID в начало файла перед запуском.
import json
import os
import logging
import re
...
ADMIN_ID = 1758979923                                      # <- твой ID
STATES_FILE = "states.json"
STARS_PROVIDER_TOKEN = "STARS"                     # <- заменишь своим провайдером
# ---------------------------------------------------------------------------
...
def payment_methods_keyboard(prefix_pack: str):
    # after chosen pack (like "private_month")
    keyboard = [
...
def russia_methods_keyboard(prefix_pack: str):
    # ---- ВАЖНОЕ ИЗМЕНЕНИЕ: FK Wallet как URL + отдельная кнопка "Инструкция" ----
    keyboard = [
        # сразу открываем ссылку при клике
        [InlineKeyboardButton("FK Wallet (Открыть)", url=FKWALLET_LINK)],
        # инструкция — при нажатии покажем пошаговую инструкцию и кнопку "Я оплатил"
        [InlineKeyboardButton("Инструкция FK Wallet", callback_data=f"{prefix_pack}_ru_fkwallet_instr")],
...
def price_display(base, dur):
    """Возвращаем строку с примерным пересчётом — статичные коэффициенты (примерные)."""
    usd = USD_PRICES[base][dur]
    # грубый пересчёт для отображения (можешь заменить)
    uah_per_usd = 42
    rub_per_usd = 93
    uah = int(usd * uah_per_usd)
    rub = int(usd * rub_per_usd)
    return f"{uah}₴ / {rub}₽ / ${usd}"

# ================== HANDLERS ==================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # send photo with main keyboard
    if update.message:
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Support (mark user as writing to support)
    if data == "support":
        pending_users[user_id] = {"state": "support"}
        save_states()
        await query.message.reply_text("🛠 Напишите своё сообщение/пришлите скрин чека. Мы перешлём его модератору.")
        return
    ...
    if m:
        ...
        # LabeledPrice expects amount in the smallest currency unit; provider 'XTR' or custom provider may expect raw int
        ...
        try:
            ...
            logging.exception("Ошибка при создании invoice: %s", e)
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_ton")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(
            f"💎 Оплата TON\nСумма: {price_display(base,dur)}\nАдрес: `{TON_ADDRESS}`\n\nСкиньте скрин чека после оплаты.",
            parse_mode="Markdown",
            reply_markup=kb)
        return
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_usdt_trc")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(
            f"💵 Оплата USDT TRC20\nСумма: {price_display(base,dur)}\nАдрес: `{USDT_TRC20}`\n\nСкиньте скрин чека после оплаты.",
            parse_mode="Markdown",
            reply_markup=kb)
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_usdt_erc")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(
            f"💵 Оплата USDT ERC20\nСумма: {price_display(base,dur)}\nАдрес: `{USDT_ERC20}`\n\nСкиньте скрин чека после оплаты.",
            parse_mode="Markdown",
            reply_markup=kb)
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_cryptobot")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(
    ...
    if m:
        prefix_pack = f"{m.group(1)}_{m.group(2)}"
        await query.message.reply_text("Способы оплаты (Казахстан):", reply_markup=kazakh_methods_keyboard(prefix_pack))
        return
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Перейти в Donatello", url=DONATELLO_LINK)],[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_uk_donatello")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(f"💰 Donatello\nСумма: {price_display(base,dur)}\nСсылка: {DONATELLO_LINK}\n\nСкиньте скрин чека после оплаты.", reply_markup=kb)
        return

    # Ukraine -> Donat Alerts
    m = re.match(r"^(private|vip)_(month|year|forever)_uk_donatalerts$", data)
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Перейти в Donat Alerts", url=DONATALERTS_LINK)],[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_uk_donatalerts")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(f"💰 Donat Alerts\nСумма: {price_display(base,dur)}\nСсылка: {DONATALERTS_LINK}\n\nСкиньте скрин чека после оплаты.", reply_markup=kb)
        return
    ...
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Перейти в Donat Alerts", url=DONATALERTS_LINK)],[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_ru_donatalerts")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(f"💰 Donat Alerts\nСумма: {price_display(base,dur)}\nСсылка: {DONATALERTS_LINK}\n\nСкиньте скрин чека после оплаты.", reply_markup=kb)
        return

    # Russia -> FK Wallet INSTRUCTION (callback)
    m = re.match(r"^(private|vip)_(month|year|forever)_ru_fkwallet_instr$", data)
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        # show instruction and a "Я оплатил" button
        instr = (f"💳 FK Wallet — инструкция\n\n"
                 f"Сумма: {price_display(base,dur)}\n\n"
                 "1) Перейдите по ссылке на FK Wallet (кнопка ниже).\n"
    ...
    # Kazakhstan -> Donat Alerts
    m = re.match(r"^(private|vip)_(month|year|forever)_kz_donatalerts$", data)
    if m:
        base, dur = m.group(1), m.group(2)
        prefix_pack = f"{base}_{dur}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Перейти в Donat Alerts", url=DONATALERTS_LINK)],[InlineKeyboardButton("✅ Я оплатил", callback_data=f"paid_{prefix_pack}_kz_donatalerts")],[InlineKeyboardButton("⬅️ Назад", callback_data="back")]])
        await query.message.reply_text(f"💰 Donat Alerts (Казахстан)\nСумма: {price_display(base,dur)}\nСсылка: {DONATALERTS_LINK}\n\nСкиньте скрин чека после оплаты.", reply_markup=kb)
        return

    # ====== Пользователь нажал "Я оплатил" ======
    if data.startswith("paid_"):
        payload = data.replace("paid_", "")
        parts = payload.split("_")
        # ожидаем: private_month_usdt_trc, private_month_ru_fkwallet, etc.
        if len(parts) < 3:
            pack = parts[0] + "_" + (parts[1] if len(parts) > 1 else "month")
            method = "_".join(parts[2:]) if len(parts) > 2 else "manual"
        else:
            pack = f"{parts[0]}_{parts[1]}"
            method = "_".join(parts[2:])
        # помечаем ожидание скрина
        ...
        try:
            await context.bot.send_message(
                ADMIN_ID,
                f"Пользователь @{username or user_id} (ID: {user_id}) отметил оплату: {pack} | способ: {method}.",
    ...
    # ADMIN: установить state для ответов (replyto_)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_users:
        ...
        if state == "awaiting_screenshot":
            ...
            keyboard = [[InlineKeyboardButton(f"Выдать ссылку @{username}", callback_data=f"give_{user_id}")]]
            try:
    ...
    # пользователь прислал фото без пометки
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # admin: reply to bot message in admin chat (parsing ID inside text/caption)
